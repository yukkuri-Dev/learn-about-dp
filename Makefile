...
BUILDS       := ja cn
EXCLUDE      :=
CFILES       := $(filter-out $(EXCLUDE),$(wildcard $(SOURCEDIR)/*.c)) $(wildcard $(SOURCEDIR)/libc/*.c) $(wildcard $(SOURCEDIR)/libct/*.c) $(wildcard $(SOURCEDIR)/libct/fsc/*.c) $(SOURCEDIR)/libct/ui/dialog/user_input_dialog.c
